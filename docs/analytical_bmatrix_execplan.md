...
- [x] (2026-05-16) Added analytical torsion rows for regular torsions with finite-difference fallback for singular or near-linear torsion geometries.
...
- Observation: Initial analytical torsion support produced above-tolerance rows for torsions around near-linear nitrile/alkyne axes in `benzonitrile.hess`, `CH3CN_freq.hess`, and `propyne.hess`.
  Evidence: 2026-05-16 full sweep after first torsion implementation reported `rows_above_tolerance_count=8`, with large deltas on torsions such as `tor(H4-C1-C2-N3)` and `tor(C2-C1-C3-H4)`.

- Observation: Adding a torsion plane-sine fallback with `torsion_sin_tol=2.0e-1` resolved those row deltas while preserving rank invariants.
  Evidence: 2026-05-16 full sweep after torsion fallback reported `rows_above_tolerance_count=0`, `files_with_redundant_rank_change=0`, `files_with_selected_rank_change=0`, `selected_basis_difference_count=10`, and `selected_basis_replacement_rank_loss_count=0`.
...
- Decision: Support analytical torsion rows only for regular torsion geometries.
  Rationale: Torsion derivatives are singular or numerically unstable when either projected torsion-plane vector is small, especially near-linear nitrile and alkyne axes. The hybrid API therefore falls back to finite differences when the torsion plane sine is at or below `2.0e-1`, preserving baseline parity and diagnostics.
  Date/Author: 2026-05-16 / Codex
...
Analytical torsion outcome:

- Added analytical torsion rows for regular four-atom torsion coordinates.
- Added `singular_or_near_linear_torsion` fallback for torsions with small projected plane vectors.
- Full sweep now reports `selected_basis_difference_count=10` because `anisole.hess` adds one rank-preserving selected-basis alternative after torsion rows are analytical.
...
The new `analytical_B(coords_A, internals)` returns a B matrix plus diagnostics. It reports per-row methods and fallback reasons. Near-linear and high-angle rows fall back to finite differences through `angle_sin_tol=2.0e-1`; near-linear torsions fall back through `torsion_sin_tol=2.0e-1`.
...
- `analytical_B(coords_A, internals, eps=EPS_FD_A, singular_tol=1.0e-12, angle_sin_tol=2.0e-1, torsion_sin_tol=2.0e-1) -> tuple[np.ndarray, dict]`
